...
function renderBookingsTable(loading) {
  ...
  tbody.innerHTML = filtered.map(function(b) {
    ...
    return '<tr><td><span class="token-badge">' + token + '</span></td><td style="font-size:12px; color:var(--text3);">' + bid + '</td><td style="font-weight:500;">' + name + '</td><td>' + email + '</td><td style="font-size:13px;">' + (b.service || 'Aadhaar Update') + '</td><td style="font-size:13px;">' + displayDate + '</td><td><span class="status-pill ' + (b.status || 'pending') + '">' + (b.status || 'pending').charAt(0).toUpperCase() + (b.status || 'pending').slice(1) + '</span></td><td><div class="action-btns">' + (b.status === 'pending' ? '<button class="action-btn" onclick="updateBookingStatus(\'' + b.id + '\',\'approved\')">Approve</button>' : '') + (b.status !== 'cancelled' && b.status !== 'completed' ? '<button class="action-btn danger" onclick="updateBookingStatus(\'' + b.id + '\',\'cancelled\')">Cancel</button>' : '') + '</div></td></tr>';
  }).join('');
}
